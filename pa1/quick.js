const { List, PairSorting } = require('immutable')
...
const fold_left = function (f, base, ls) {
  ...
  let first_element = ls.first();
  let new_list = ls.shift();

  return fold_left(f, f(base, first_element), new_list); // recursive fold_left call
};
...
const map = (g, ls) => fold_left((curr_acc, x) => curr_acc.push(g(x)), List([]), ls);
// g() takes the current accumulator and adds the newly mapped element onto the new list
...
const filter = (g, ls) => fold_left((curr_acc, x) => (g(x) ? curr_acc.push(x) : curr_acc), List([]), ls);
// function passed into fold_left() checks to see if element passes the condition:
// true = add element onto new list, false = do not change current list

/* More readable code for future reference
{
  fcn = (curr_acc, x) => (g(x) ? curr_acc.push(x) : curr_acc)
  return fold_left(fcn, List([]), ls)
}
*/
...
const partition = (g, ls) => {
  ...
  const list1 = filter(g, ls); // filter based on numbers that eval to true
  const list2 = filter((x) => !g(x), ls); // filter based on numbers that eval to false
...
const quicksort = function (ls) {
  ...
  // partition with elements <= pivot and elements > pivot
  let part_list = partition((x) => x <= pivot, new_list);

  // recursively call quicksort
  let left_part = quicksort(part_list.get(0));
  let right_part = quicksort(part_list.get(1));

  // combine pivot with two partitions
  let final_list = List([]);
  return final_list.concat(left_part, pivot, right_part);
};
...
// assert(quicksort(List([4, 7, 3, 6, 8, 7, 1, 2])).equals(List([1, 2, 3, 4, 6, 7, 7, 8])));

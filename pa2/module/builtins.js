...
function getBuiltIn(name) {
  ...
  // Calling the function should return the module object, loading it via require() if necessary.
  return thunk();
}
 ...
 * names.  
 * Specifically, for each built-in module name `name` we will set
 * `cache[name]` to a function which should return the corresponding module
 * object. 
 * If the module has not already been loaded with require(), this
 * function should do so. 
 * Importantly, however, it should only call require() once for any name. 
 * So, for example, the incorrect (but very close) solution is:
 *
...
// _.each(names, name => {
//   cache[name] = (() => {
//     let module; // Initialize a variable to store the loaded module.
//     console.log(!module)

//     return () => {
//       if (!module) {
//         module = require(name); // Load the module if it hasn't been loaded yet.
//       }

...
function loadJSONFile(resolvedName) {
  ...
  if (require_._cache[resolvedName]) {
    return require_._cache[resolvedName]
  } else {
    const jsonData = fs.readFileSync(resolvedName, 'utf8')
 ...
 *
 * Specifically, let's redefine require_.loader to handle '.json' files. 
 * Note that the default require_.loader handles '.js' files. We want to allow for
...
require_.loader = (function (origLoader) {

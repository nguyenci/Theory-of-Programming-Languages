...
listReverse :: [a] -> [a]
-- listReverse xs = if null xs then [] else listReverse (tail xs) ++ [head xs]
listReverse xs
  | null xs = []
...
sqSum xs = foldLeft f base xs
  where
    f a x = a + (x * x) -- a is the accumulator
    base = 0
...
sepConcat sep (h : t) = foldLeft f base l
 where
    f a x
      | null a && null x = sep -- if a and x are null/"", return sep
      | otherwise = a ++ sep ++ x
    base = h
    l = t
...
bigAdd l1 l2 = removeZero res
  where
    ...
    f (x1, x2) (carry, sum) = (newCarry, digit : sum)
      where
        total = x1 + x2 + carry -- add corresponding elts of l1 and l2 and carry
        digit = total `mod` 10 -- second digit of the total
        newCarry = total `div` 10 -- first digit of the total
    base = (0, [])
    args = (0,0) : zip l1' l2' -- produces a list of tuples
...
mulByDigit i l = removeZero res
  where
    ...
    f x (carry, prod) = (newCarry, newProd)
      where
      total = i * x + carry -- multiply an elt of list with i, then add carry
      newCarry = total `div` 10 -- first digit of the total
      newProd -- add second digit to the beginning of prod list
        | (length prod == length l - 1) && (newCarry /= 0) = [newCarry, total `mod` 10] ++ prod
        | otherwise = total `mod` 10 : prod
    base = (0, [])
    args = l
...
bigMul l1 l2 = res
    where
    (_, res) = foldRight f base args
    f x (z, p) = (add_zeroes, total) 
      where
        add_zeroes = z ++ [0] -- add a zero to the end
        prod = mulByDigit x l1 ++ z -- multiply l1 by an elt of l2, then add trailing zeroes
        total = bigAdd p prod -- add the current total to the new product
    base = ([],[])
    args = l2
